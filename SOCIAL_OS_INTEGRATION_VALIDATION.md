...
- Sincronização de main em 30/08/2026: confirmada a chegada do commit `18786a2d5e28e15412e919485ec5d547b22ca3bb` (“Segurança de fluxo: rotas legadas removidas, dupla revisão e rate limit de IA”). Antes da publicação, a migration aditiva `0020_automation_allow_self_approval.sql` foi revisada e aplicada no TiDB. A coluna `automation_settings.allowSelfApproval` foi confirmada como `tinyint(1) NOT NULL DEFAULT 1`; não houve remoção ou alteração de dados.
- Validação pós-sincronização: `pnpm check`, `pnpm test` (86 aprovados e 1 teste externo Meta ignorado), `pnpm build`, `pnpm audit --prod --audit-level high` e `pnpm deploy:preflight` foram aprovados. O preflight confirmou `needsMigration=false`. O smoke test da produção respondeu 200 em health, readiness e nas 14 rotas principais, com contrato JSON íntegro.
...
O pipeline Woodpecker associado ao PR #10 é uma validação independente do commit atual de main. Ele permanece aprovado na fila normal do agente único da VPS compartilhada, sem deploy ou migration, conforme detalhado em `WOODPECKER_CI_INTEGRATION_STATUS.md`.
