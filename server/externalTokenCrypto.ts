...
export function decryptExternalToken(provider: ExternalTokenProvider, value: string) {
  ...
  if (version !== VERSION || storedProvider !== provider || !ivPart || !tagPart || !encryptedPart) throw new Error("A credencial externa armazenada não possui formato compatível com o provedor solicitado.");
  try {
