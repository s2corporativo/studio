...
function sign(provider: ExternalOAuthProvider, value: string) {
  return createHmac("sha256", secret(provider)).update(value).digest("base64url");
}

export function createExternalOAuthState(userId: number, provider: ExternalOAuthProvider, profileId?: number | null) {
  const payload: ExternalOAuthStatePayload = {
    userId,
