  ...
  status: protectedProcedure.query(async ({ ctx }) => {
    ...
    const integrations = buildExternalIntegrationStatuses(getExternalIntegrationConfigFromEnv(), instagramConnection).map(item => {
      ...
      return {
        ...
        connected,
        missingConfiguration: facebookConfigured ? [] : ["App ID", "App Secret", "Graph API version"],
        state: connected ? "connected" as const : pending ? "ready_for_oauth" as const : facebookConfigured ? "configured_unvalidated" as const : "awaiting_credentials" as const,
        detail: connected
          ...
          : pending
            ...
            : facebookConfigured
              ...
              : "O conector de Facebook Pages está implementado e aguarda App ID, App Secret e versão da Graph API no ambiente protegido.",
