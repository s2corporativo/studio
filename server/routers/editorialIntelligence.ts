...
export const editorialIntelligenceRouter = router({
  overview: protectedProcedure.query(async ({ ctx }) => {
    ...
    const recent = studio.posts.slice(0, 20);
  ...
  refreshRadarOpportunities: protectedProcedure.input(z.object({ limit: z.number().int().min(1).max(8).default(5) })).mutation(async ({ ctx, input }) => {
    const [radar, studio, existing] = await Promise.all([
      fetchCurrentRadar(),
    ...
    for (const item of candidates) {
      const [assessment, heuristic] = await Promise.all([
        ...
        Promise.resolve(scoreEditorialPotential({
          ...
          hook: item.summary,
          area: item.area,
          audience: studio.brand?.targetAudience,
          format: "carousel",
          sourceUrl: item.url,
      ...
      created.push({ ...row, freshnessStatus: item.freshnessStatus, validUntil: item.validUntil, engagementScore: heuristic.engagementScore });
    }
    ...
    return { created: created.length, blocked: blocked.length, opportunities: created };
  }),
});
