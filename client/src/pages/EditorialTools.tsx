...
function TemplatePreview({ title, hook, brandName, template }: { title?: string; hook?: string; brandName?: string; template: string }) {
...
export function ContentDeskV4(props: any) {
  ...
  useEffect(() => {
    if (selectedTopic) setForm((current: any) => ({ ...current, topicId: selectedTopic.id, area: selectedTopic.area, topic: selectedTopic.title, audience: selectedTopic.audience, format: selectedTopic.suggestedFormat, legalSource: selectedTopic.sourceUrl ?? "" }));
  }, [selectedTopic?.id]);
  ...
  const similar = useMemo(() => {
    if (!editor) return [];
    const currentTopic = topics.find((topic: any) => topic.id === editor.topicId)?.title ?? "";
    const candidate = `${currentTopic} ${editor.title ?? ""} ${editor.caption ?? ""}`;
    return posts.filter((post: any) => {
      if (post.id === editor.id) return false;
      const postTopic = topics.find((topic: any) => topic.id === post.topicId)?.title ?? "";
  ...
  const selectSource = (value: string, target: "form" | "editor") => {
    const source = sources.find((item: any) => item.id === Number(value));
    if (target === "form") setForm({ ...form, sourceId: value ? Number(value) : null, legalSource: source?.url ?? form.legalSource });
    else setEditor({ ...editor, sourceId: value || "", legalSource: source?.url ?? editor.legalSource });
  };
    ...
    <div className="space-y-6">
      <section className="editorial-panel rounded-2xl p-5 sm:p-6">
        ...
        <form className="mt-6 space-y-4" onSubmit={createDraft}>
          <Field label="Tema da biblioteca"><select className="editorial-input" value={form.topicId ?? ""} onChange={(event) => onSelectTopic(Number(event.target.value))}>{topics.map((topic: any) => <option className="bg-[#12221e]" value={topic.id} key={topic.id}>{topic.area} — {topic.title}</option>)}</select></Field>
          ...
          <div className="grid gap-3 sm:grid-cols-2"><Field label="Pilar editorial"><select className="editorial-input" value={form.contentPillar} onChange={(event) => setForm({ ...form, contentPillar: event.target.value })}>{pillars.map((item) => <option className="bg-[#12221e]" key={item}>{item}</option>)}</select></Field><Field label="Estágio do funil"><select className="editorial-input" value={form.funnelStage} onChange={(event) => setForm({ ...form, funnelStage: event.target.value })}>{stages.map((item) => <option className="bg-[#12221e]" key={item} value={item}>{item}</option>)}</select></Field></div>
...
export function KnowledgeDeskV2({ materials, onAdd, adding, onUpload, uploading }: { materials: any[]; onAdd: (value: any) => void; adding: boolean; onUpload: (value: any) => void; uploading: boolean }) {
