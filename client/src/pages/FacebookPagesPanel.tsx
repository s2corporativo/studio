...
export default function FacebookPagesPanel() {
  ...
  const selectPage = trpc.facebookPages.selectPage.useMutation({ onSuccess: async page => { setNotice(`Página ${page.accountName ?? page.externalAccountId} selecionada.`); await Promise.all([utils.facebookPages.status.invalidate(), utils.externalIntegrations.status.invalidate(), utils.socialStudio.socialProfiles.invalidate()]); }, onError: error => setNotice(error.message) });
