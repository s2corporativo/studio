...
import { AlertTriangle, BarChart3, Bot, Building2, Inbox, Lightbulb, Loader2, RefreshCw, Sparkles, Target, Users } from "lucide-react";
...
export default function SocialOsCommandCenter() {
  ...
  const classifyInteraction = trpc.socialIntelligence.classifyInteraction.useMutation({
    onSuccess: async result => {
      await Promise.all([utils.socialOs.interactions.invalidate(), utils.socialOs.dashboard.invalidate(), utils.socialGrowth.workspace.invalidate()]);
      toast.success(result.interaction.requiresHumanApproval ? "Interação classificada e encaminhada para revisão humana." : "Interação classificada pela IA.");
    },
    onError: error => toast.error(error.message),
  });
  ...
  return <div className="studio-command-center space-y-6">
    ...
    <div className="grid gap-6 xl:grid-cols-[1.3fr_.7fr]">
      ...
      <div className="space-y-6">
        <Card className="border-[#daba7d]/10 bg-[#10201b] text-[#f3ebdd]"><CardHeader><CardTitle className="flex items-center gap-2 text-lg"><Inbox className="h-5 w-5 text-[#e2ba7c]" />Inbox inteligente</CardTitle></CardHeader><CardContent className="space-y-3">{(interactions.data ?? []).slice(0, 6).map(item => <div key={item.id} className="rounded-xl border border-white/[.06] p-3"><div className="flex items-center justify-between gap-2"><span className="text-sm font-medium">{item.authorName ?? item.authorHandle ?? "Contato"}</span><Badge variant="outline">{item.kind}</Badge></div><p className="mt-2 line-clamp-3 text-xs leading-5 text-[#9aa89f]">{item.body}</p>{item.aiSuggestedReply && <p className="mt-2 rounded-lg bg-white/[.03] p-2 text-[11px] leading-5 text-[#b9c2bc]">Sugestão IA: {item.aiSuggestedReply}</p>}<div className="mt-2 flex items-center justify-between gap-2">{item.requiresHumanApproval ? <p className="text-[11px] text-amber-300">Requer revisão humana</p> : <span />}<Button size="sm" variant="ghost" disabled={classifyInteraction.isPending} onClick={() => classifyInteraction.mutate({ id: item.id })}><Sparkles className="mr-1 h-3.5 w-3.5" />Classificar IA</Button></div></div>)}{!interactions.data?.length && <p className="text-sm text-[#8f9c93]">Nenhuma interação recebida.</p>}</CardContent></Card>
