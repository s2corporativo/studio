...
type VideoPlatform = "instagram" | "facebook" | "linkedin" | "tiktok" | "youtube";
type SeoScope = "technical" | "site" | "content" | "local";
type AdsPlatform = "meta" | "google" | "youtube" | "linkedin" | "tiktok";
type ReportType = "weekly" | "monthly" | "campaign" | "executive";
type VideoForm = { title: string; platform: VideoPlatform; durationSeconds: number; audience: string; objective: string; source: string };
type SeoForm = { targetUrl: string; scope: SeoScope; location: string; keyword: string };
type AdsForm = { platform: AdsPlatform; name: string; objective: string; audience: string; location: string; offer: string; budget: string; durationDays: number; landingPageUrl: string };
type ReportForm = { reportType: ReportType; periodStart: string; periodEnd: string };
...
export default function GrowthWorkspace() {
  ...
  const [video, setVideo] = useState<VideoForm>({ title: "", platform: "instagram", durationSeconds: 45, audience: "", objective: "Autoridade e educação", source: "" });
  const [seo, setSeo] = useState<SeoForm>({ targetUrl: "https://", scope: "technical", location: "", keyword: "" });
  const [ads, setAds] = useState<AdsForm>({ platform: "meta", name: "", objective: "Leads qualificados", audience: "", location: "", offer: "", budget: "", durationDays: 30, landingPageUrl: "" });
  const defaultEnd = useMemo(() => new Date(), []);
  const defaultStart = useMemo(() => { const value = new Date(); value.setDate(value.getDate() - 30); return value; }, []);
  const [report, setReport] = useState<ReportForm>({ reportType: "monthly", periodStart: localDateInput(defaultStart), periodEnd: localDateInput(defaultEnd) });
  ...
  if (location === "/video") items = <List rows={videos} empty="Nenhum projeto de vídeo criado." render={row => <><strong>{row.title}</strong><small>{row.platform} · {row.durationSeconds}s · {row.status}</small>{row.hook && <p className="mt-2 text-xs leading-5 text-[#a8b3ab]">Gancho: {row.hook}</p>}</>} />;
  else if (location === "/seo") items = <List rows={seoRows} empty="Nenhuma auditoria SEO registrada." render={row => <><strong>{row.scope.toUpperCase()} · {row.keyword ?? row.targetUrl ?? "Auditoria"}</strong><small>Score {row.score}/100 · {row.status}</small></>} />;
  else if (location === "/ads") items = <List rows={adRows} empty="Nenhum plano de mídia criado." render={row => <><strong>{row.name}</strong><small>{row.platform} · {row.objective} · {row.status} · aprovação obrigatória</small></>} />;
  else if (location === "/agentes") items = <List rows={agents} empty="Nenhuma execução de agente registrada." render={row => <><strong>{row.agentType}</strong><small>{row.status}{row.entityType ? ` · ${row.entityType}` : ""}{row.durationMs ? ` · ${row.durationMs} ms` : ""}</small>{row.errorMessage && <p className="mt-2 text-xs text-rose-300">{row.errorMessage}</p>}</>} />;
  else if (location === "/memoria") items = <List rows={memory} empty="A memória estratégica ainda não possui aprendizados." render={row => <><strong>{row.title}</strong><small>{row.memoryType} · confiança {row.confidenceScore}%</small><p className="mt-2 text-xs leading-5 text-[#a8b3ab]">{row.content}</p></>} />;
  else if (location === "/governanca") items = <List rows={compliance} empty="Nenhuma checagem de compliance registrada." render={row => <><strong>{row.checkType}</strong><small>{row.result} · {row.checkedBy}</small></>} />;
  else items = <List rows={reports} empty="Nenhum relatório gerado." render={row => <><strong>{row.reportType}</strong><small>{new Date(row.periodStart).toLocaleDateString("pt-BR")} → {new Date(row.periodEnd).toLocaleDateString("pt-BR")}</small><p className="mt-2 text-xs leading-5 text-[#a8b3ab]">{row.summary}</p></>} />;
  ...
  return <div className="space-y-6">
    ...
    {(location === "/video" || location === "/seo" || location === "/ads" || location === "/relatorios") && <Card className="border-[#daba7d]/10 bg-[#10201b] text-[#f3ebdd]">
      ...
      <CardContent>
        {location === "/video" && <form className="grid gap-4 md:grid-cols-2" onSubmit={(event: FormEvent) => { event.preventDefault(); videoMutation.mutate({ postId: null, title: video.title, platform: video.platform, durationSeconds: Number(video.durationSeconds), audience: video.audience, objective: video.objective, source: video.source || null, tone: null }); }}>
          <Field label="Tema"><input required value={video.title} onChange={e => setVideo({ ...video, title: e.target.value })} /></Field>
          <Field label="Plataforma"><select value={video.platform} onChange={e => setVideo({ ...video, platform: e.target.value as VideoPlatform })}><option value="instagram">Instagram</option><option value="facebook">Facebook</option><option value="linkedin">LinkedIn</option><option value="tiktok">TikTok</option><option value="youtube">YouTube</option></select></Field>
        ...
        {location === "/seo" && <form className="grid gap-4 md:grid-cols-2" onSubmit={(event: FormEvent) => { event.preventDefault(); seoMutation.mutate({ targetUrl: seo.targetUrl, scope: seo.scope, location: seo.location || null, keyword: seo.keyword || null }); }}>
          ...
          <Field label="Escopo"><select value={seo.scope} onChange={e => setSeo({ ...seo, scope: e.target.value as SeoScope })}><option value="technical">Técnico</option><option value="site">Site</option><option value="content">Conteúdo</option><option value="local">SEO Local</option></select></Field>
        ...
        {location === "/ads" && <form className="grid gap-4 md:grid-cols-2" onSubmit={(event: FormEvent) => { event.preventDefault(); adsMutation.mutate({ platform: ads.platform, name: ads.name, objective: ads.objective, audience: ads.audience, location: ads.location || null, offer: ads.offer || null, budgetCents: ads.budget ? Math.round(Number(ads.budget.replace(",", ".")) * 100) : null, durationDays: Number(ads.durationDays), landingPageUrl: ads.landingPageUrl || null }); }}>
          <Field label="Nome do plano"><input required value={ads.name} onChange={e => setAds({ ...ads, name: e.target.value })} /></Field>
          <Field label="Plataforma"><select value={ads.platform} onChange={e => setAds({ ...ads, platform: e.target.value as AdsPlatform })}><option value="meta">Meta Ads</option><option value="google">Google Ads</option><option value="youtube">YouTube Ads</option><option value="linkedin">LinkedIn Ads</option><option value="tiktok">TikTok Ads</option></select></Field>
        ...
        {location === "/relatorios" && <form className="grid gap-4 md:grid-cols-3" onSubmit={(event: FormEvent) => { event.preventDefault(); reportMutation.mutate({ reportType: report.reportType, periodStart: new Date(`${report.periodStart}T00:00:00`), periodEnd: new Date(`${report.periodEnd}T23:59:59`) }); }}>
          <Field label="Tipo"><select value={report.reportType} onChange={e => setReport({ ...report, reportType: e.target.value as ReportType })}><option value="weekly">Semanal</option><option value="monthly">Mensal</option><option value="campaign">Campanha</option><option value="executive">Executivo</option></select></Field>
    ...
    {insights.length > 0 && <Card className="border-[#daba7d]/10 bg-[#10201b] text-[#f3ebdd]"><CardHeader><CardTitle className="text-lg">Aprendizados recomendados</CardTitle></CardHeader><CardContent className="space-y-3">{insights.slice(0, 5).map(item => <div key={item.id} className="rounded-xl border border-white/[.06] p-4"><div className="flex items-start justify-between gap-3"><strong className="text-sm">{item.title}</strong><span className="text-xs text-[#e2ba7c]">{item.confidenceScore}%</span></div><p className="mt-2 text-xs leading-5 text-[#9aa89f]">{item.recommendation}</p></div>)}</CardContent></Card>}
  </div>;
}
...
function List<T extends { id: number }>({ rows, empty, render }: { rows: T[]; empty: string; render: (row: T) => React.ReactNode }) {
