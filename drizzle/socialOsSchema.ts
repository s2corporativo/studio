...
export const contentOpportunities = mysqlTable("content_opportunities", {
  ...
  locality: varchar("locality", { length: 180 }),
  ...
  commercialScore: int("commercialScore").default(0).notNull(),
  riskScore: int("riskScore").default(0).notNull(),
  totalScore: int("totalScore").default(0).notNull(),
  rationale: text("rationale"),
  status: mysqlEnum("status", opportunityStatuses).default("new").notNull(),
  detectedAt: timestamp("detectedAt").defaultNow().notNull(),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
  updatedAt: timestamp("updatedAt").defaultNow().onUpdateNow().notNull(),
}, table => [
  index("content_opportunities_user_score_idx").on(table.userId, table.totalScore),
  index("content_opportunities_user_status_idx").on(table.userId, table.status),
]);
...
export const postVersions = mysqlTable("post_versions", {
  ...
  uniqueIndex("post_versions_post_version_unique").on(table.postId, table.version),
  index("post_versions_user_post_idx").on(table.userId, table.postId),
]);
...
export const postApprovalBindings = mysqlTable("post_approval_bindings", {
  ...
  index("post_approval_bindings_post_idx").on(table.userId, table.postId),
]);
...
export const campaignRuns = mysqlTable("campaign_runs", {
  ...
  postsPerWeek: int("postsPerWeek").notNull(),
  timezone: varchar("timezone", { length: 80 }).default("America/Sao_Paulo").notNull(),
  status: mysqlEnum("status", ["planning", "generated", "failed"]).default("planning").notNull(),
  generatedCount: int("generatedCount").default(0).notNull(),
  errorMessage: text("errorMessage"),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
  updatedAt: timestamp("updatedAt").defaultNow().onUpdateNow().notNull(),
}, table => [
  uniqueIndex("campaign_runs_idempotency_unique").on(table.idempotencyKey),
  index("campaign_runs_user_idx").on(table.userId, table.createdAt),
]);
...
export const socialInteractions = mysqlTable("social_interactions", {
  ...
  authorHandle: varchar("authorHandle", { length: 180 }),
  body: text("body").notNull(),
  kind: mysqlEnum("kind", interactionKinds).default("question").notNull(),
  status: mysqlEnum("status", interactionStatuses).default("open").notNull(),
  ...
  index("social_interactions_user_status_idx").on(table.userId, table.status),
  index("social_interactions_user_kind_idx").on(table.userId, table.kind),
]);
...
export const leads = mysqlTable("leads", {
  ...
  sourceInteractionId: int("sourceInteractionId"),
...
export const competitors = mysqlTable("competitors", {
  ...
  index("competitors_user_idx").on(table.userId, table.active),
]);
...
export const contentMetrics = mysqlTable("content_metrics", {
  ...
  index("content_metrics_user_post_idx").on(table.userId, table.postId),
]);
...
export const creativeEvaluations = mysqlTable("creative_evaluations", {
  ...
  visualQuality: int("visualQuality").default(0).notNull(),
  brandFit: int("brandFit").default(0).notNull(),
  legibility: int("legibility").default(0).notNull(),
  attentionPotential: int("attentionPotential").default(0).notNull(),
  aiAppearanceRisk: int("aiAppearanceRisk").default(0).notNull(),
  notes: text("notes"),
  passed: boolean("passed").default(false).notNull(),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
}, table => [
  index("creative_evaluations_user_post_idx").on(table.userId, table.postId),
]);
...
export const automationRules = mysqlTable("automation_rules", {
  ...
  triggerType: mysqlEnum("triggerType", automationTriggerTypes).notNull(),
  triggerConfigJson: text("triggerConfigJson").notNull(),
  actionType: mysqlEnum("actionType", automationActionTypes).notNull(),
  actionConfigJson: text("actionConfigJson").notNull(),
  requiresHumanApproval: boolean("requiresHumanApproval").default(true).notNull(),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
  updatedAt: timestamp("updatedAt").defaultNow().onUpdateNow().notNull(),
}, table => [
  index("automation_rules_user_idx").on(table.userId, table.enabled),
]);
